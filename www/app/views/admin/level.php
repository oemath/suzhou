  ...
  src="http://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS-MML_HTMLorMML">
	...
	<script>
		...
		function saveStatus(pid) {
			$.cookie('oemathcookie-level-pid', pid);
		}
...
//    		            $("#prob-info").text("");
		...
		function onsaveLevel(level) {
		    ...
		    $.ajax({
		        ...
		        success: function (data, textStatus, jqXHR) {
		            if (data.status == "OK") {
						$("#prob-info").text("Saved level="+level+"; pid="+curr_pid);
						$('#prob-level').val(level);
						onclickNext();
						success = true;
		            }
		            else {
			            $("#prob-info").text("/insert/level FAILED("+data.error+"):" + jqXHR+","+textStatus);
		            }
		        },
		        failure: function (jqXHR, textStatus, errorThrown) {
		            $("#prob-info").text("/insert/level failure:" + jqXHR+","+textStatus+","+errorThrown);
		        },
		        error: function (jqXHR, textStatus, errorThrown) {
		            $("#prob-info").text("/insert/level error:" + jqXHR+","+textStatus+","+errorThrown);
		        },
		    });
		    if (!success) alert('Save level failed!');
			return success;
		}
...
//			$('#prob-cid').val(data.cid);
		...
		function process_pids(pids)
		{
			...
			if (pid_list.length > 0) {
				curr_pid = $.cookie('oemathcookie-level-pid');
		...
		$(function() {
		    ...
		    $(window).keydown(function(event) {
		    	if(event.ctrlKey) {
			    	...
			    	if ('1' <= event.key && event.key <= '5') {
			    		onsaveLevel(event.key - '0');
				    	event.preventDefault();
				    	return false;
			    	}
		    	}
